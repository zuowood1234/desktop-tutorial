...
for i, line in enumerate(lines):
    if 'if page == "📊 实时分析":' in line:
        break_line = i
        break

if break_line == -1:
    print("❌ Fatal Error: Could not find 'if page == \"📊 实时分析\":'")
    exit(1)

# 保留上半部分 (包含 break_line)
header_lines = lines[:break_line]

# 构建新的 content
new_content_code = r'''if page == "📊 实时分析":
