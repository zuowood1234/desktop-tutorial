...
def run_compare_backtest(symbol, days=90):
    ...
    # 预计算全表的均线 (用于参考，但在循环中只给 AI 看到今日之前的)
    df_all['MA5'] = df_all['收盘'].rolling(window=5).mean()
    df_all['MA10'] = df_all['收盘'].rolling(window=10).mean()
    df_all['MA20'] = df_all['收盘'].rolling(window=20).mean()
    
    pos_pure = False 
    pos_sent = False
    
    print(f"🧠 开始严格逐日回测 (共 {total_len} 个交易日) [含 MA 趋势参考]...")
    ...
    for i in range(start_index, total_len - 1):
        ...
        for item in recent_data:
            d = item['date']
            ma_info = f"MA5:{item['ma5']:.2f}|MA20:{item['ma20']:.2f}" if not pd.isna(item['ma20']) else "计算中"
            line = f"{d} | 收:{item['close']:.2f} | 涨:{item['pct']:.2f}% | {ma_info}"
            k_lines_text += line + "\n"
        ...
        def build_prompt(strategy_name, data_text, is_holding):
            ...
            action_guide = ""
            if is_holding:
                action_guide = "你已入场。若均线上行且价在均线上，建议【持有】；若跌破MA5或出现巨量阴线，考虑【卖出】止盈。"
            else:
                action_guide = "你正寻找机会。若价在MA5/MA20之上且放量上涨，应果断【买入】顺势而为，不要因涨幅大而盲目恐高！若缩量下跌则继续【观望】。"
            
            return f"""
你是 A 股短线势能交易员。{status_str}
基于最近 10 天行情(含均线趋势)，判断【今天】({date_str}) 的操作：

{data_text}

交易哲学：顺势而为。在多头趋势(价>MA5>MA20)中应积极参与，不要轻易下车或恐高。

要求：请严格按照格式输出：操作|理由(5-10字)
操作：【买入】/【卖出】/【持有】/【观望】
...
"""
        ...
        try:
            ...
            reason = parts[1].strip() if len(parts) > 1 else "趋势持续"
            ...
            logging.error(f"技术派失败 {date_str}: {e}")
        ...
        try:
            ...
            parts = content.split('|')
            action = parts[0].strip()
            reason = parts[1].strip() if len(parts) > 1 else "情绪稳定"
            ...
            logging.error(f"情绪派失败 {date_str}: {e}")
